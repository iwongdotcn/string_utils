...
struct tokenizer
{
	...
	tokenizer(string_t const& _str, string_t const& _delimiters)
		: offset_(0), string_(_str), delimiters_(_delimiters) {
	}
	tokenizer(string_t const& _str, typename string_t::value_type const* _delimiters)
...
split(std::basic_string<char_t, traits_t, allocator_t> const& __string,
	...
	for (tokenizer<string_type> token(__string, __delimiters); token.next_token(); ) {
...
split(std::basic_string<char_t> const& __string,
	...
	return split(__string, detail::make_safe_string(__delimiters));
}
